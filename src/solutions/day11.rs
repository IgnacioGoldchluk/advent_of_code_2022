...
impl Monkey {
    ...
    fn parse_test(monkey_info: &str) -> Box<dyn Fn(u64) -> bool> {
        let divisor = monkey_info
            .lines()
    ...
    pub fn perform_round(&mut self) -> Vec<(usize, u64)> {
        let mut operations: Vec<(usize, u64)> = Vec::new();
        self.items.iter().for_each(|item| {
            let worry_level = (self.operation)(*item) / 3;
            let to_throw = self.throw.get(&(self.test)(worry_level)).unwrap();
            operations.push((*to_throw, worry_level));
            self.inspected += 1;
        });
        self.items = vec![];
        operations
...
pub fn solution() {
    ...
    for _round in 0..20 {
        for idx in 0..monkeys.len() {
            ...
            let pushes = monkey.perform_round();
